...
export function buildResendQuizTemplateHtml(key: TemplateKey): string {
	...
	return buildEmailHtmlInner(RESEND_FIRST_NAME, RESEND_CTA_HREF, {
		...tpl,
		tag,
	});
}
...
export const TEMPLATES: Record<TemplateKey, TemplateConfig> = {
	"lw-autoestima": {
		...
		highlight:
			"&ldquo;A maioria das pessoas não falha por falta de esforço. Falha por falta de estrutura e de um ambiente que as mantenha no caminho.&rdquo;",
	...
	"lw-saude": {
		...
		highlight:
			"Nossos professores trabalham com alunos em diferentes fases de saúde, do zero à alta performance. Você não vai ser tratado(a) como um número.",
	...
	"lw-evento": {
		...
		highlight:
			"Em 4 semanas de treino consistente, a maioria dos alunos já reporta diferença visível nas roupas, mesmo antes da balança mostrar.",
		ctaLabel: "Começar antes que o tempo passe →",
		ctaNote:
			"Atendemos de segunda a sábado. Responda esse email ou chame no WhatsApp.",
	},
	...
	"gm-confianca": {
		...
		highlight:
			"Alunos que treinam com acompanhamento profissional evoluem, em média, 3x mais rápido do que quem treina sozinho.",
		ctaLabel: "Quero começar minha transformação →",
	...
	"en-trabalho": {
		...
		highlight:
			"Você não precisa de muito tempo. Precisa de consistência. E a Panobianco Satélite está aqui pra garantir isso.",
	...
	"he-medico": {
		subject:
			"{Nome}, ouvir o médico foi a decisão mais importante que você tomou",
		tag: "Resultado do seu quiz",
		...
		highlight:
			"Você não precisa chegar aqui em forma. Você precisa chegar. O resto a gente constrói junto.",
	...
	ebook: {
		...
		highlight:
			"E quando as coisas melhorarem e você quiser dar o próximo nível, a Panobianco Satélite vai estar esperando por você, com braços abertos e uma oferta especial pra quem já é da família.",
...
export function selectTemplate(
	...
	if (goal === "energy") {
		...
		if (energyMissing === "self" || energyMissing === "hobbies")
			return "en-autoestima";
		return "en-trabalho";
	}
